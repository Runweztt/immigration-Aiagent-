...
from src.llm.circuit_breaker import is_claude_available, is_ollama_available, is_openai_available
...
class LLMFactory:
    ...
    _llm_type_map: ClassVar[dict[LLMType, LLMConfig]] = {
        ...
        LLMType.CLOUD_FAST: {"provider": "anthropic", "model": "claude-sonnet-4-20250514", "temperature": 0.7},
        LLMType.CLOUD_ACCURATE: {"provider": "anthropic", "model": "claude-sonnet-4-20250514", "temperature": 0.5},
    }
    ...
    def get_llm_for_task(cls, task_type: str, sensitive_data: bool = False, temperature: float | None = None) -> BaseLLM:
        ...
        if provider == "anthropic" and not is_claude_available():
            logger.warning("Claude not available, falling back to local model")
        ...
        try:
            ...
            # If all else fails, try to create a basic Claude model
            try:
                logger.info("Falling back to basic Claude model")
                return cls.create_llm_by_provider("anthropic", "claude-sonnet-4-20250514", temperature or 0.7)
            except Exception as e:
...
def get_llm(provider: str | None = None, temperature: float = 0.7) -> BaseLLM:
    ...
    Args:
        provider: The LLM provider to use (ANTHROPIC, OPENAI, or OLLAMA)
    ...
    if provider == "anthropic" and not is_claude_available():
        logger.warning("Claude not available, falling back to Ollama")
        provider = "ollama"
    ...
    if provider == "ollama" and not is_ollama_available():
        logger.warning("Ollama not available, falling back to Claude")
        provider = "anthropic"

    if provider == "anthropic":
        model = os.getenv("ANTHROPIC_MODEL_NAME", "claude-sonnet-4-20250514")
        return LLMFactory.create_llm_by_provider("anthropic", model, temperature)
